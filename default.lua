--Texts
strings = {

-------------	Explorer	----------------------------------------------
back =			"Return ",
copyfile =		"Copying: ",
delfile =		"Deleting: ",
dir =			"<DIR>",
creatfolder =	"Create Folder",
newfolder =		"New Folder",
pass =			"Use Pass ?",
ospass =		"PASS",

--SubMenu
copy =			"Copy",
delete =		"Delete",
makedir =		"MakeDir",
rename =		"Rename",
extract =		"Extract",
cancel =		"Cancel",
insvpkfromdir = "Install Game",
instheme =		"Install CTheme",
extractto =		"Extract To",
paste =			"Paste",
move =			"Move",
size =			"Size",
scanvpks =		"ScanVpks",
export =		"Export",
themes =		"Themes AppMan",
pluginsman =	"Plugins Man",
cthemesman =	"CustomThemes",

items =			"Sel Items: ",

------------	Ftp 		-----------------------------------------------
ftp =			"Enter this FTP addres to your device or PC",
closeftp =		"Press START to close FTP conection",
loseftp =		"FTP Conection Closed",

imgerror =		"Error Image not supported",

-- ## Music Player ##
playing =		"Playing",
paused =		"Paused",
time =			"Time: ",
tags =			"xx:xx:xx",
sounderror =	"Error Sonido Invalido",

--AppManager
apptittle =		"Game Manager (Apps - Bubbles)",
switch =		"Really wish to switch ",
nomemory =		"Not enough memory...",
appremove =		"Really wish to Uninstall ",
delclon	=		"Delete this CLON: ",
updatedb =		"Your PSVita will restart...\nand your database will be rebuilt",
manual =		"Do you really want to remove this manual",
notmanual =		"Manual has been removed!",
notfindmanual =	"Manual not exist",
removemanual =	"Remove manual",
pressremove =	"Uninstall",
pressswitch =	"Swicht (ux0<->ur0)",
empty =			"Empty/Error",
toexplorer =	"Explorer",

--SearchingVpks
vpktittle =		"Vpks Install",
moveiso2ux0 =	"Move Game to ux0:pspemu/ISO",
moveiso2ur0 =	"Move Game to ur0:pspemu/ISO",

-------------		Callbacks		---------------------------------------------------------------------
unsafe =		"Unsafe: ",
file =			"File: ",
percent =		"Percent: ",
percent_total = "Percent Total: ",
total_size =	"Total Size: ",
total_written = "Total Written: ",
--onAppInstall
searchunsafe =	"Search in vpk, Unsafe or Dangerous files!",
unsafevpk =		"Extended permissions and personal info access required! \nUse caution with non trusted source packages! \nDo you want to install at your own risk?",
dangerousvpk =	"Warning!!! High risk of bricking your device \nif this VPK is installed! \nUse caution with non trusted source packages! \nDo you want to install at your own risk?",
vpkunpack =		"Unpack vpk...",
install =		"Installing...",

--onExtractFiles
extraction =	"Extraction...",

--onScanningFiles
scanning =		"Scanning...",
size_unpk =		"Size Unpack: ",
-----------------------------------------------------------------------------------------------------------

--Launch PBP or VPK
launchpbp =		"Launch game/app ",
confirm =		"X: Accept  |  O: Cancel",
warningvpk =	"\nWARNING TOTAL ACCESS\nUNSAFE VPK",
warningvpk2 =	"WARNING TOTAL ACCESS",
safevpk =		"VPK SAFE",
total_sizevpk = "Total Size VPK: ",
installvpk =	"INSTALL VPK",
installgame =	"INSTALL APP",
alertunsafe =	"VPK UNSAFE!!!!",
alertdang =		"VPK DANGEOROUS!!!!",
installvpkdir =	"INSTALL VPK FROM DIR",
launchbin =		"Launch app ",

------------	Switch Apps/vpks	------------------------------------------------------------------------
required =		"\nRequired: ",
searchapp =		"\nWould you like to switch an app to ur0 ?",
vpktour0 =		"Would you like to move the vpk to ur0: so you can install it ?",
vpktoux0 =		"Would you like to move the vpk to ux0: ?",
stillnomemory =	"Not enough space even if you do the switch",
vpkinuro =		"The vpk is at ur0: already...\nTry to free up some more space...",
apptour0 =		"Switch to ur0:",
ur0toapp =		"Switch app to ux0: ?",
tittle =		"Title: ",
errorapp =		"Error...app can not be moved...",
errorinstall =	"An error ocurred when installing",

------------	ThemesAppmanager	-------------------------------------------------------------------------
themesappman =	"Themes for AppManager PLUS",
themesdone =	"Installation Completed!",
reload =		"Reload Original Theme",
settinsthemes =	"Go to Settings Theme ?",

------------	PluginsManager		-------------------------------------------------------------------------
plugsmanager =	"Plugins Manager",
reloadconfig =	"TaiHen Config",
configtxt =		"Reload taiHEN ConfigTxt",
disablesuprx =	"Disable plugin ?",

------------	ThemesLivearea		-------------------------------------------------------------------------
customthemes =	"CustomThemes LiveArea",
notthemes =		"Empty/Error in theme list!",
deltheme =		"Uninstall CustomTheme from app.db",
delfilestheme =	"Delete files from CustomTheme ?",

result =		"\nResult: ",
count =			"Count: ",
sizeis =		"Size is: ",
openmusic =		"Open Mp3 Player/Gallery ?",
filesfolders =	"Files/Folders",

}